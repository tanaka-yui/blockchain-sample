...
func NewWalletTransaction(privateKey *ecdsa.PrivateKey, publicKey *ecdsa.PublicKey,
	sender string, recipient string, value float32) *WalletTransaction {
	return &WalletTransaction{privateKey, publicKey, sender, recipient, value}
}

func (t *WalletTransaction) GenerateSignature() *ecdsautil.Signature {
	m, _ := json.Marshal(t)
	h := sha256.Sum256(m)
